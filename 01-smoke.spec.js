...
async function chooseIndiaUsdLocale(page) {
  ...
  // Main-site locale control renders country and currency in separate labels.
  // Do not wait on #localeCurrent: that element exists in some calculator pages,
  // but not on the main Learn page.
  await expect(page.locator('#localeCountryLabel')).toHaveText('India', { timeout: 5000 });
  await expect(page.locator('#localeCurrencyLabel')).toHaveText('USD', { timeout: 5000 });
